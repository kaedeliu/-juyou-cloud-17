...
<#if swagger>
@Schema(title="${entity}AddDto对象", description="${entity}AddDto对象")
</#if>
...
<#list table.fields as field>
    ...
    <#-- 要新增的字段才写入-->
    <#if customField?? && customField!="" && customField.addColumns?? && customField.addColumns==1>
        <#if field.keyFlag>
    ...
    @NotNull<#if customField.fieldDesc!?length gt 0>(message = "请填写${customField.fieldDesc}")</#if>
            <#else>
    @NotNull<#if field.fieldDesc!?length gt 0>(message = "请填写${customField.fieldDesc}")</#if>
            </#if>
        </#if>
        <#--正则-->
